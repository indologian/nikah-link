...
-- Changing to another theme still requires an active published version and entitlement.
...
begin
  ...
  if v_theme_changed then
    if not exists (select 1 from public.themes t where t.id=p_theme_id and t.is_active=true) then raise exception 'Selected theme is not active'; end if;
    if not public.user_can_use_theme(auth.uid(),p_theme_id) then raise exception 'Theme requires an active paid plan'; end if;
    select tv.id,tv.component_key into v_effective_theme_version_id,v_version_component_key
    from public.theme_versions tv
    where tv.id=p_theme_version_id and tv.theme_id=p_theme_id and tv.is_published=true and tv.lifecycle_status='published';
