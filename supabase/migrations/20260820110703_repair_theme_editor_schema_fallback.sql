-- Mirror renderer registry fields into the theme metadata fallback.
...
RETURNS public.theme_versions LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
...
begin
  ...
  if jsonb_array_length(v_fields_schema) = 0 then select coalesce(t.fields_schema,'[]'::jsonb) into v_fields_schema from public.themes t where t.id = p_theme_id; end if;
