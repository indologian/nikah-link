...
AS $$
  WITH profile AS (
    SELECT role, plan, has_used_free_trial, plan_expires_at
    FROM public.profiles
    WHERE user_id = p_user_id
  ), usage AS (
    SELECT count(*)::integer AS invitation_count
...
CREATE POLICY "Users can create invitations within plan" ON public.invitations
FOR INSERT TO authenticated WITH CHECK (
  (SELECT auth.uid()) = user_id
  AND public.can_create_invitation((SELECT auth.uid()))
);
...
begin
  ...
  if new.theme_id is not null and theme_changed then
    if not exists (select 1 from public.themes t where t.id = new.theme_id and t.is_active = true) then
      raise exception 'Selected theme is not active';
    end if;
    if auth.uid() is not null and not public.user_can_use_theme(auth.uid(), new.theme_id) then
      raise exception 'Theme requires an active paid plan';
    end if;
  end if;

  if tg_op = 'UPDATE' and new.theme_id = old.theme_id and new.theme_version_id is null then
    new.theme_version_id := old.theme_version_id;
  end if;

  if new.theme_id is not null and new.theme_version_id is not null then
    select tv.theme_id, tv.fields_schema into resolved_theme_id, version_fields
  ...
  if version_fields is not null and jsonb_typeof(version_fields) = 'array' then
    for field in select value from jsonb_array_elements(version_fields) loop
      ...
      if field_required and field_enabled
         and (new.custom_data is null or nullif(btrim(coalesce(new.custom_data->>field_name, '')), '') is null) then
...
-- update_invitation remains an owner-only RPC but now permits saving an existing
-- invitation pinned to an archived theme; changing theme still requires an active
-- published version and entitlement. See current function definition in production.
