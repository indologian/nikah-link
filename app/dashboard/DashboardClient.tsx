...
const PlanBadge = ({ plan }: { plan: string }) => {
  if (plan === "pro") {
    return (
      <span className="flex items-center gap-1 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest">
        PRO VIP
      </span>
    );
  }
  ...
  if (plan === "premium") {
    return (
      <span className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest">
        PREMIUM
      </span>
    );
  }
  ...
  return (
    <span className="flex items-center gap-1 bg-transparent border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest">
      FREE
    </span>
  );
};
...
function StatCard({ icon: Icon, label, value, change }: {
  ...
  return (
    <div className="p-5 bg-transparent border border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col justify-between hover:border-[var(--accent-rosegold)] transition-colors group">
      ...
      <div>
        <div className="text-3xl font-playfair font-bold text-[var(--text-primary)] dark:text-white leading-none">{value}</div>
        {change && (
    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 pb-6 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-[var(--text-primary)] dark:text-white font-serif text-lg">
            {getInitials(displayName)}
          </div>
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-2xl font-playfair font-bold text-[var(--text-primary)] dark:text-white">{displayName}</h1>
              <PlanBadge plan={plan} />
            </div>
        ...
        <Link
          ...
          className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[var(--accent-rosegold)] text-white px-6 py-3 rounded-full font-semibold text-sm hover:bg-[var(--accent-rosegold-hover)] transition-colors"
        >
          ...
          {QUICK_ACTIONS.map((action) => {
            ...
            return (
              <Link
                key={action.label}
                href={action.href}
                className="flex flex-col items-center justify-center gap-3 p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 hover:border-[var(--accent-rosegold)] hover:bg-white dark:hover:bg-slate-900 transition-all text-center group"
              >
                <Icon className="w-5 h-5 text-slate-400 group-hover:text-[var(--accent-rosegold)] transition-colors" />
                <span className="text-slate-600 dark:text-slate-300 text-xs font-semibold uppercase tracking-wider group-hover:text-[var(--accent-rosegold)] transition-colors">
                  {action.label}
                </span>
              </Link>
            );
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          {hasInvitations && (
            <Link href="/dashboard/undangan" className="text-[var(--accent-rosegold)] text-xs font-bold uppercase tracking-wider hover:underline transition-colors">
              Lihat Semua
            </Link>
          )}
        </div>
        ...
        {!hasInvitations ? (
          <div className="flex flex-col items-center justify-center py-20 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 bg-transparent text-center px-4">
            ...
            <Link
              href="/dashboard/undangan/baru"
              className="inline-flex items-center justify-center gap-2 bg-[var(--accent-rosegold)] text-white px-6 py-2.5 rounded-full font-semibold text-xs hover:bg-[var(--accent-rosegold-hover)] transition-colors"
            >
              <Plus className="w-4 h-4" /> Buat Undangan
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {invitations.slice(0, 3).map((inv) => (
              <div
                key={inv.id}
                className="group flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden hover:border-[var(--accent-rosegold)] transition-all"
              >
                <div className="p-6 pb-4 border-b border-slate-100 dark:border-slate-800 flex-1">
                  ...
                  <h3 className="font-playfair font-bold text-xl text-[var(--text-primary)] dark:text-white truncate mb-1">
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-900/50 flex flex-wrap sm:flex-nowrap gap-2">
                  <Link
                    href={`/${inv.username}`}
                    target="_blank"
                    className="flex-1 py-2 text-center text-xs font-semibold text-[var(--text-primary)] dark:text-white bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-[var(--accent-rosegold)] transition-colors flex items-center justify-center gap-1.5"
                  >
